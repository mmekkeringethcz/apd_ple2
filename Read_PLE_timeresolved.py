# -*- coding: utf-8 -*-
"""
Created on Mon Jan 13 13:09:36 2020
...
folder = 'C:/Users/rober/Documents/Doktorat/Projects/SingleParticle_PLE/Andor Spectrometer/20200110_CryoDots_HR63/'
# folder = 'E:/LAB_DATA/Robert/20200110_CryoDots_HR63/'
namelist = ['QD3_PLE']    #Paste the file that ends in _tttrmode.out in here without the .out
...
Wavelength_calib=np.array([578.8,560.3,541.8,523.4,504.9])
...
reffolder='C:/Users/rober/Documents/Doktorat/Projects/SingleParticle_PLE/Andor Spectrometer/20200110_CryoDots_HR63/'
# reffolder='E:/LAB_DATA/Robert/20200110_CryoDots_HR63/'
calibspec=importASC(reffolder+'RefSpec_2p29MHz_200Hz_260mVpp_n20mVoff.asc')
...
#%% Plot spectrum in eV
plt.figure()
plt.plot(1240/Wavelengthspec,ylistspec/interprefspec(Wavelengthspec))
plt.ylim((0, 6000))
plt.xlim((2.1,2.415))
plt.xlabel('Excitation Wavelength (eV)')
plt.ylabel('Intensity')
plt.title(namelist[0])
...
tlistspec = (xlistspec[:-1]+0.5*(xlistspec[1]-xlistspec[0]))*dtmacro
plt.plot(1240/tlistspec,ylistspec/interprefspec(tlistspec))
plt.xlim((2.1,2.415))
plt.xlabel('Excitation Wavelength (eV)')
plt.ylabel('Intensity')
plt.title(namelist[0])
...
plt.figure()
mtimelims=np.array([18,30])
binning=1
mtimelimsdtmic=np.round(mtimelims/data[0]*1e-9)
nbins=mtimelimsdtmic[1]-mtimelimsdtmic[0]
plt.hist2d(data[2]*data[0]*1e9,calibcoeffs[1]+InVoltage(data[19]*data[1],Freq,Vpp,Voffset,Verror)*calibcoeffs[0],range=[[mtimelimsdtmic[0]*data[0]*1e9,mtimelimsdtmic[1]*data[0]*1e9],[500,580]],bins=[(nbins-1)*binning,50],norm=mcolors.LogNorm())
plt.xlabel('Time (ns)')
plt.ylabel('Excitation wavelength (nm)')
plt.colorbar()
#%% Ex spectrum vs time
plt.figure()
plt.hist2d(data[3]*data[1],calibcoeffs[1]+InVoltage(data[19]*data[1],Freq,Vpp,Voffset,Verror)*calibcoeffs[0],range=[[1,Texp],[520,595]],bins=[100,160])#,norm=mcolors.LogNorm())
plt.xlabel('Time (s)')
plt.ylabel('Excitation wavelength (nm)')
plt.colorbar()
...
#    print(tbinnr)
    ...
    if len(microtimes)>30:
        [taulist[tbinnr],Alist[tbinnr],ybglist[tbinnr],buff[tbinnr]]=GetLifetime(microtimes,dtmicro,dtmacro,110e-9,tstart=lifetime[3]*dtmicro,histbinmultiplier=1,ybg=0*lifetime[2]*binwidth/Texp,plotbool=False,method='ML_c') 
    else:
...
#%% Plot FLID map
plt.figure()
plt.hist2d(tauav,photonspbin,(50,np.max(photonspbin)/2),range=[[0,50],[0,np.max(photonspbin)]])#,norm=mcolors.LogNorm())
plt.title('FLID map')
...
#    elif photonspbin[tbinnr]>limittrlow and photonspbin[tbinnr]<limittrhigh and taulist[tbinnr]>0.5 and (photonspbin[tbinnr]-10)/taulist[tbinnr]>505/27:
        ...
        bins_trion+=1
        nrtrion+=nphots
#    elif photonspbin[tbinnr]>limittrlow and photonspbin[tbinnr]<limittrhigh and tauav[tbinnr]>6 :
...
fittrion=GetLifetime(microtimes_trion,dtmicro,dtmacro,10e-9,tstart=-1,plotbool=True,ybg=0*lifetime[2]*bins_trion/(Texp/binwidth),method='ML_c')
...
#plt.xlim([0,220])
#plt.legend(['High cps','High cps fit','Low cps','Low cps fit'])
...
plt.xlim([525,wavellimit])
#plt.ylim([0,20])
plt.xlabel('Excitation Wavelength (nm)')
plt.ylabel('(normalized) Counts')
plt.legend(['Bright','Dark'])
